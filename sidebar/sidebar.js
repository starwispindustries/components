...
const CustomIcon = ({ children, ...props }) => {
    return (
        <Icon viewBox="0 0 20 20" fill="inherit" w="20px" h="20px" {...props}>
            {children}
        </Icon>
    );
};
...
const SidebarContent = ({ isDark }) => {
    ...
    return (
        <VStack mt="29px" spacing="9px">
            ...
            >
                <CustomIcon viewBox={"0 0 24 24"} h={6} w={6}>
                    <path
                        d={
                            colorMode == "light"
...
const Sidebar = ({ variant, isDark }) => {
    ...
    const borderColor = useColorModeValue("primary.light.200", "primary.dark.200");
    const { isOpen, onOpen, onClose } = useDisclosure();

    return variant === "sidebar" ? (
        <Box position="fixed" left={0} p={5} w="84px" top={0} h="100%" bg={bg} zIndex={1}>
